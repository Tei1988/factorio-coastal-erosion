...
const checkTiles: Array<string> = ['stone-path', 'refined-concrete', 'refined-hazard-concrete-left', 'refined-hazard-concrete-right']
...
const updateCoastTilePoint = function(position: Position, point: number) {
  ...
  if (point > 0) {
    ...
    if (c === undefined) { global.coastCount = global.coastCount + 1 }
    coastMap[x][y] = point
  } else {
